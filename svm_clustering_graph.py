...
# --- Get GNN embeddings for all test data ---
...
# --- Get GNN embeddings for healthy training data (for One-Class SVM fitting) ---
train_embedding, _ = get_embeddings(model, training_list)
train_embeddings_flat = train_embedding.mean(axis=1).reshape(-1, 1).astype(np.float64)
...
# Fit One-Class SVM on healthy GNN embeddings
oc_svm = OneClassSVM(kernel='rbf', gamma='scale', nu=0.1)
oc_svm.fit(train_embeddings_scaled)

# Predict on all test data: +1 = inlier (healthy/0), -1 = outlier (different/1)
svm_predictions = oc_svm.predict(embeddings_scaled)
# Convert SVM output: +1 -> 0 (healthy), -1 -> 1 (different)
cluster_labels = np.where(svm_predictions == 1, 0, 1)
...
res_df = pd.DataFrame({"File": ["svm_clustering_graph.py"], "Technique": ["GCN + Feature Attention + One-Class SVM"], "Accuracy": [acc], "Precision": [prec], "Recall": [rec]})
try:
