...
from torch_geometric.nn import GCNConv, global_mean_pool, GINEConv, SAGEConv
...
X = embeddings.reshape(-1, 1).astype(np.float64)

# --- Build training embeddings from healthy data for One-Class SVM ---
train_embedding = get_embeddings(model, training_list)
X_train = train_embedding.mean(axis=1).reshape(-1, 1).astype(np.float64)
...
# Fit One-Class SVM on healthy training embeddings
...
res_df = pd.DataFrame({"File": ["graphsage_svm.py"], "Technique": ["GraphSAGE + One-Class SVM"], "Accuracy": [acc], "Precision": [prec], "Recall": [rec]})
try:
